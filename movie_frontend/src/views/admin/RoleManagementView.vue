...
async function fetchData() {
  loading.value = true
  try {
    const [rl, pm] = await Promise.all([adminApi.getAllRoles(), adminApi.getAllPermissions()])
    roles.value = rl
...
async function handleAddRole() {
  const name = prompt('Tên vai trò mới (VD: MODERATOR):')
  if (!name) return
  const description = prompt('Mô tả vai trò:') || ''
  saving.value = true
  try {
...
async function handleAddPermission() {
  const name = prompt('Tên quyền hạn mới (VD: DELETE_COMMENT):')
  if (!name) return
  const description = prompt('Mô tả quyền hạn:') || ''
  saving.value = true
  try {
      ...
      <div class="grid-layout">
        <section class="manage-section">
          <div class="section-top">
            ...
            <button @click="handleAddRole" :disabled="saving" class="add-btn">+</button>
          </div>
          <div class="list-card">
        ...
        <section class="manage-section">
          <div class="section-top">
            ...
            <button @click="handleAddPermission" :disabled="saving" class="add-btn">+</button>
          </div>
          <div class="list-card">
