  ...
  <AdminLayout>
    <div class="page">
      ...
      <div class="reviews-grid">
        ...
        <div v-else v-for="r in reviews.filter(x => !search || x.content?.toLowerCase().includes(search.toLowerCase()) || (x.user?.username || '').toLowerCase().includes(search.toLowerCase()))" :key="r.id" class="review-card">
      ...
      <div v-if="totalPages > 1" class="pagination">
        <button @click="page = Math.max(1, page - 1); fetchReviews()" :disabled="page === 1" class="page-btn">‹</button>
        <button v-for="p in totalPages" :key="p" @click="page = p; fetchReviews()" :class="['page-btn', page === p ? 'active' : '']">{{ p }}</button>
        <button @click="page = Math.min(totalPages, page + 1); fetchReviews()" :disabled="page === totalPages" class="page-btn">›</button>
      </div>
    </div>
  </AdminLayout>
</template>
...
.search-box svg { width: 14px; height: 14px; color: #52525b; }
...
.review-card { background: #18181b; border: 1px solid rgba(255,255,255,0.06); border-radius: 16px; padding: 18px; display: flex; flex-direction: column; }
.review-top { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 12px; }
.review-user { display: flex; align-items: center; gap: 10px; }
.user-avatar { width: 36px; height: 36px; border-radius: 50%; background: #7c3aed; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 14px; color: white; }
.username { font-size: 13.5px; font-weight: 600; color: #f4f4f5; }
.review-date { font-size: 11px; color: #52525b; }
.review-rating { font-size: 13px; font-weight: 700; color: #fbbf24; background: rgba(251,191,36,0.1); padding: 2px 8px; border-radius: 6px; }

.review-content { font-size: 13.5px; color: #a1a1aa; line-height: 1.6; min-height: 50px; }
.review-footer { margin-top: 14px; display: flex; justify-content: flex-end; }
.delete-btn { font-size: 12px; font-weight: 600; color: #f87171; background: none; border: 1px solid rgba(239,68,68,0.2); padding: 4px 10px; border-radius: 6px; cursor: pointer; transition: all 0.15s; }
.delete-btn:hover { background: rgba(239,68,68,0.1); }

.review-skeleton { background: #18181b; border: 1px solid rgba(255,255,255,0.06); border-radius: 16px; height: 160px; animation: pulse 1.5s infinite; }
