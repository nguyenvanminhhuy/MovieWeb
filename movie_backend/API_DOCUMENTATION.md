...
*Dành cho người xem phim, không yêu cầu đăng nhập.*
...
## 11. Thống kê & Phân quyền (Admin Dashboard & RBAC) - 7 API

| STT | Phương thức | Endpoint | Chức năng chi tiết |
| :--- | :--- | :--- | :--- |
| 72 | `GET` | `/admin/stats/dashboard` | Thống kê tổng quan Dashboard cho Admin. |
| 73 | `POST` | `/roles` | Tạo vai trò mới (Admin, User, VIP...). |
| 74 | `GET` | `/roles` | Danh sách các vai trò trong hệ thống. |
| 75 | `DELETE` | `/roles/{role}` | Xóa vai trò. |
| 76 | `POST` | `/permissions` | Tạo quyền hạn mới. |
| 77 | `GET` | `/permissions` | Danh sách tất cả quyền hạn hiện có. |
| 78 | `DELETE` | `/permissions/{permission}` | Xóa quyền hạn. |
